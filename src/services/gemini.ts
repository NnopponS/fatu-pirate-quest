...
export const chatWithPirate = async (
  ...
  if (userContext) {
    ...
    if (userContext.points !== undefined && userContext.pointsRequired !== undefined) {
      ...
      if (remaining > 0) {
        userContextText += `\n- ต้องการอีก ${remaining} คะแนนเพื่อเขย่าขวด`;
      } else {
        userContextText += `\n- สะสมคะแนนครบแล้ว! สามารถเขย่าขวดได้`;
      }
    }
    ...
    if (userContext.hasSpun) {
      ...
      if (userContext.points >= userContext.pointsRequired) {
        userContextText += `\n- ยังไม่ได้เขย่าขวด (สามารถเขย่าได้แล้ว!)`;
      } else {
        userContextText += `\n- ยังไม่ได้เขย่าขวด (ต้องสะสมคะแนนให้ครบก่อน)`;
      }
    }
  }
...
- ได้คะแนนครบ 300 คะแนนสามารถเขย่าขวดสมบัติรับรางวัล (เขย่า 5 ครั้ง)
- มีกิจกรรมย่อยต่างๆ ในแต่ละสถานที่ที่สามารถทำเพื่อรับคะแนนเพิ่ม (+100 ต่อกิจกรรม)
- ผู้เข้าร่วมสามารถเช็กอินสถานที่ + ทำกิจกรรมย่อยได้
...
ตัวอย่างการตอบ:
- "คะแนนของเจ้าตอนนี้คือ 150 คะแนน ยังขาดอีก 150 เพื่อเขย่าขวดสมบัติ! ⚓💪 📊 (ข้อมูลจากระบบของข้า)"
...
- "เจ้าได้เช็กอินแล้ว ${userContext.checkedInLocations?.length || 0} จาก ${userContext.totalLocations || 0} สถานที่ และทำกิจกรรมย่อยไปแล้ว ${userContext.completedSubEvents || 0} กิจกรรม! 📊 (ข้อมูลจากระบบของข้า)"
- "เมื่อเจ้าเขย่าขวด 5 ครั้ง จะพบกับรางวัลที่สุ่มออกมา! แต่ละคนเขย่าได้แค่ครั้งเดียวเท่านั้นนะ 🍾💎 📊 (ข้อมูลจากระบบของข้า)"`;
